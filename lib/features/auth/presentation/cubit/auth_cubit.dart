...
class AuthCubit extends Cubit<AuthState> {
  ...
  AuthCubit(this._registerUseCase, this._signInUseCase, this._signOutUseCase) : super(AuthInitial());

  void signUpWithEmailAndPassword(
      String email, String password, UserType userType) async {
    emit(const AuthStateLoading());
    final result =
        await _registerUseCase(RegisterParams(email: email, password: password, userType: userType));
    result.fold(
