...
class SplashDataSourceImpl implements SplashDataSource {
  ...
  Future<UserModel> findUser(String id) async {
    try {final user = await _firebaseFirestore.collection('users').doc(id).get();
    if (!user.exists) {
    ...
    if (userData['userType'] == 'parent') {
      ...
      throw ApiException(dialogTextCode: e.message ?? 'unknown', dialogTitleCode: e.code, statusCode: 500);
    }
  }
}
